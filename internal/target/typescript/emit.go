...
func validateNPMPackageName(name string) error {
	if !npmPackageNamePattern.MatchString(name) {
		return fmt.Errorf("TypeScript package name %q is not a valid npm package name", name)
	}
	return nil
}
...
func PackageArtifacts(document *ir.Document, pkg Package) ([]Artifact, error) {
	...
	if err := validateNPMPackageName(pkg.Name); err != nil {
		return nil, err
	}
	if !semverPattern.MatchString(document.ContractVersion) {
		return nil, fmt.Errorf("TypeScript package version %q is not valid SemVer", document.ContractVersion)
	}
	...
	if err := validatePackageExportSymbols(map[string][]byte{
		"types":  typesSource,
		"errors": errorsSource,
		"client": clientSource,
	}); err != nil {
...
func validatePackageExportSymbols(modules map[string][]byte) error {
	...
	for _, module := range moduleNames {
		...
		for symbol := range exports {
			if previous, exists := owners[symbol]; exists && previous != module {
				return fmt.Errorf("generated package export %q is declared by both %s and %s modules", symbol, previous, module)
			}
