...
func emitTypes(document *ir.Document) ([]byte, error) {
	...
	enumCatalogs := make([]enumCatalogPlan, 0)
	for _, schemaName := range names {
		...
		enumCatalogs = append(enumCatalogs, enumCatalogPlan{
			name:           schemaName,
...
`)
	...
	enumBindings := make([]runtimeProperty, 0, len(enumCatalogs))
	for _, catalog := range enumCatalogs {
		...
		enumBindings = append(enumBindings, runtimeProperty{key: catalog.name, value: catalog.catalogBinding})
	}
