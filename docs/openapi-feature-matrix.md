...
| Fetch-managed request headers | all | all | all | Generated client projection and pre-dispatch policy; full inbound contract generated with `--with server` | omit Fetch-forbidden caller inputs, reject typed/raw injection, preserve server validation | `internal/target/typescript/runtime_parity_test.go::TestRuntimeEnforcesFetchManagedRequestHeaderPolicy` |
