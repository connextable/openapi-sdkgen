...
| Fetch-managed request headers | all | all | all | Generated client projection and pre-dispatch policy; Webhook/Callback inbound contracts generated with `--with server` | omit Fetch-forbidden caller inputs, reject typed/raw injection, preserve generated inbound validation | `internal/target/typescript/runtime_parity_test.go::TestRuntimeDelegatesEnvironmentControlledRequestHeadersToFetch` |
...
| Link request-header expressions for Fetch-managed names | all | Error with the exact Link parameter JSON Pointer | prevent unobservable source reads and unassignable target values | `internal/target/typescript/runtime_parity_test.go::TestGeneratedResponseLinksRejectHostManagedRequestHeaders` |
...
| Fetch-managed header API keys | all | Generated header API-key credentials follow Fetch-managed fixed-name and conditional method-value policy before transport | pre-dispatch header credential policy | `internal/target/typescript/runtime_parity_test.go::TestRuntimeDelegatesEnvironmentControlledHeaderAPIKeysToFetch` |
