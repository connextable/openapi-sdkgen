...
| Fetch-managed header API keys | all | Generated header API-key credentials follow Fetch-managed fixed-name and conditional method-value policy before transport | pre-dispatch header credential policy | `internal/target/typescript/runtime_parity_test.go::TestRuntimeEnforcesFetchManagedPolicyForHeaderAPIKeys` |
