...
| Fetch-managed header API keys | all | Header API-key credentials follow Fetch-managed fixed-name and conditional method-value policy before transport | pre-dispatch header credential policy | `internal/target/typescript/runtime_parity_test.go::TestRuntimeEnforcesFetchManagedPolicyForHeaderAPIKeys` |
