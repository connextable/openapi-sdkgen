...
while [ "$#" -gt 0 ]; do
  ...
  case "$arg" in
    ...
    patch|minor|major)
      ...
      ;;
		v*)
			if [[ ! "$arg" =~ $TAG_REGEX ]]; then
				ui_error "Version tag must be valid SemVer: $arg"
				exit 1
			fi
			TAG_INPUT="$arg"
			;;
...
validate_prerelease() {
  ...
  for identifier in "${identifiers[@]}"; do
    if [[ "$identifier" =~ ^[0-9]+$ && "$identifier" =~ ^0[0-9]+$ ]]; then
      ui_error "Numeric prerelease identifiers must not have leading zeroes: $tag"
      exit 1
    fi
  done
}
...
semver_is_greater() {
  ...
  local candidate_prerelease baseline_prerelease candidate_identifier baseline_identifier index=0

  read -r candidate_major candidate_minor candidate_patch <<<"$(semver_from_tag "$candidate")"
  read -r baseline_major baseline_minor baseline_patch <<<"$(semver_from_tag "$baseline")"
  if ((candidate_major != baseline_major)); then ((candidate_major > baseline_major)); return; fi
  if ((candidate_minor != baseline_minor)); then ((candidate_minor > baseline_minor)); return; fi
  if ((candidate_patch != baseline_patch)); then ((candidate_patch > baseline_patch)); return; fi
  candidate_prerelease="$(prerelease_part "$candidate")"
  baseline_prerelease="$(prerelease_part "$baseline")"
  if [[ -z "$candidate_prerelease" ]]; then [[ -n "$baseline_prerelease" ]]; return; fi
  ...
  while :; do
    ...
    if [[ "$candidate_identifier" == "$baseline_identifier" ]]; then index=$((index + 1)); continue; fi
    if [[ "$candidate_identifier" =~ ^[0-9]+$ && "$baseline_identifier" =~ ^[0-9]+$ ]]; then ((10#$candidate_identifier > 10#$baseline_identifier)); return; fi
    [[ "$candidate_identifier" =~ ^[0-9]+$ ]] && return 1
    [[ "$baseline_identifier" =~ ^[0-9]+$ ]] && return 0
    [[ "$candidate_identifier" > "$baseline_identifier" ]]; return
  done
}
...
LATEST_TAG="$(get_latest_stable_tag || true)"
LATEST_RELEASE_TAG="$(get_latest_tag || true)"
...
if [ -z "$TAG_INPUT" ]; then
  if [ -z "$LATEST_RELEASE_TAG" ]; then
    ...
    read -r BASE_MAJOR BASE_MINOR BASE_PATCH <<<"$(semver_from_tag "$LATEST_RELEASE_TAG")"
    ...
    if [ -n "$RECOMMENDED_REASON" ]; then ui_kv "Reason" "$RECOMMENDED_REASON"; fi
    select_bump
...
case "$TAG_INPUT" in
  ...
  "") ;;
  *)
		if [[ ! "$TAG_INPUT" =~ $TAG_REGEX ]]; then
      ui_error "Tag must be valid SemVer or one of: patch, minor, major"
      exit 1
    fi
    validate_prerelease "$TAG_INPUT"
    if [ -n "$LATEST_RELEASE_TAG" ] && ! semver_is_greater "$TAG_INPUT" "$LATEST_RELEASE_TAG"; then
      ui_error "Tag must be newer than reachable tag $LATEST_RELEASE_TAG: $TAG_INPUT"
      exit 1
    fi
    PATCH_TAG="$TAG_INPUT"
    ;;
